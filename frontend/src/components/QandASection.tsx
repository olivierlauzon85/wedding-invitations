...
const QandASection: React.FC = () => {
  ...
  return (
    <section id="qa" className="section-wrapper bg-autumn-beige/10">
        ...
        <div className="max-w-3xl mx-auto">
          {categories.map((category, idx) => (
            <Collapsible key={idx} className="mb-6">
              <CollapsibleTrigger className="w-full flex items-center justify-between p-4 bg-autumn-sage rounded-t-lg font-serif font-medium text-xl text-white">
                {t(category.name)}
                <ChevronDown className="h-5 w-5 transition-transform duration-200" />
              </CollapsibleTrigger>
              <CollapsibleContent className="bg-white/80 backdrop-blur-sm rounded-b-lg shadow-md p-4">
